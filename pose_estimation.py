...
mtx = np.array(doc['camera_matrix']['data']).reshape(3,3)
dist = np.array(doc['distortion_coefficients']['data'])
...
class image_converter:
    ...
    def callback(self,data):
        ...
        if ret == True:
            ...
            rvec, tvec, inliers = cv2.solvePnPRansac(objpoints, corners, mtx, dist)
            print("rvecs:")
            print(rvec)
            print("tvecs:")
            print(tvec)
            ...
            cv2.line(tempimg, tuple(imgpts[0]), tuple(imgpts[1]),[255,0,0],3)  #BGR
            cv2.line(tempimg, tuple(imgpts[0]), tuple(imgpts[2]),[0,255,0],3)
            cv2.line(tempimg, tuple(imgpts[0]), tuple(imgpts[3]),[0,0,255],3)
